...
import { CreateProductDto, UpdateProductDto } from '@avans-nx-workshop/backend/dto';
...
export class ProductService {
    ...
    async getAll(): Promise<IProduct[]> {
        ...
        const items = await this.productModel.find().populate('winkel');
...
//---
    ...
    async getOne(id: string): Promise<IProduct | null> {
        ...
        const item = await this.productModel.findOne({ id: id }).populate('shop').exec();
    ...
    async getAllProductsBySearchTerm(searchTerm:string){
        return (await this.getAll()).filter(product=> product.nameProduct.toLowerCase().includes(searchTerm.toLowerCase()))
    }
...
async createProduct(productDto: CreateProductDto): Promise<IProduct> {
    const { id, shop, ...productWithoutShop } = productDto;
  
    // Voeg de schrijver expliciet toe aan het boek
    const productData = {
      ...productWithoutShop,
      shop: shop, // Hier de schrijver direct toevoegen
    };
...
async update(productId: string, updateProductDto: UpdateProductDto): Promise<IProduct> {
